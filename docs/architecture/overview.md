...
| `PreviewPointsCalculator` | Adjusts the preview line's endpoint purely for display — moves it from a struck bubble's future *center* to its actual rendered surface, at any approach angle — without touching where a shot truncates or lands. See `features/core-gameplay/firing-and-snapping.md`. |
